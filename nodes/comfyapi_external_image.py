...
class ComfyAPIDeployExternalImage:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
    ...
    def load_image(param_name, keep_alpha_channel, default_value=None, display_name=None, description=None):
        ...
        if input_image and input_image.startswith('http'):
            print(f"ComfyAPI: Fetching image from url: {input_image}")
            ...
            for i in range(retry_count):
                ...
                try:
                    if not is_valid_url(input_image):
                         print(f"ComfyAPI: Invalid image url provided. {input_image}")
                    ...
                    if response.status_code == 200:
                        ...
                        print(f"ComfyAPI: Success Load image from url: {input_image}")
                        break
                    else:
                        print(f"ComfyAPI: Failed to retrieve the image, status code: {response.status_code}")
                except Exception as e:
                    print(f'Warning({have_retry}): ComfyAPI download image URL failed. Image URL: {input_image}. Error: {e}')
            ...
            print('ComfyAPI: Input image is empty, use default image')
            return [default_value]

        if not return_image:
            raise RuntimeError(f'Error: ComfyAPI load image failed. input_image: {input_image}')
...
NODE_CLASS_MAPPINGS = {"ComfyAPIDeployExternalImage": ComfyAPIDeployExternalImage}
NODE_DISPLAY_NAME_MAPPINGS = {"ComfyAPIDeployExternalImage": "External Image (ComfyAPI Deploy)"}
